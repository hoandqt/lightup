...
$tags = $data['tags'] ?? '';
$type = $data['type'] ?? '';
$includeContent = filter_var($data['generate_content'] ?? false, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? false;
...
if ($type == 'playlist') {
    ...
    if (!$title || !$description || !$tags || !$type) {
        echo json_encode(['success' => false, 'message' => 'Missing input data.']);
        exit;
    }
}
...
try {
    ...
    if ($includeContent) {
        ...
        preg_match('/(?:\*\*|)New Content:\n+([\s\S]*)(?:\s*<\/[^>]+>)?\s*(.*?)$/', $generatedText, $newContentMatch); // Ending with $
       ...
       preg_match('/(?:\*\*|)Meta Title:(?:\*\*|)\s*(.*?)\n/', $generatedText, $metaTitleMatch);
       preg_match('/(?:\*\*|)Meta Description:(?:\*\*|)\s*(.*?)\n/', $generatedText, $metaDescriptionMatch);
       preg_match('/(?:\*\*|)Meta Keywords:(?:\*\*|)\s*(.*?)$/', $generatedText, $metaKeywordsMatch); // Ending with $
